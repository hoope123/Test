...
async function withTempFiles(inputBuffer, extension, processFn) {
  const tempInput = `mayel/temp/temp_${Date.now()}.input`;
  const tempOutput = `mayel/temp/temp_${Date.now()}.${extension}`;
...
async function formatAudio(buffer) {
  const inputPath = `mayel/temp/temp_in${Date.now()}.mp3`;
  const outputPath = `mayel/temp/temp_out${Date.now()}.mp3`;
...
async function formatVideo(buffer) {
  const inputPath = `mayel/temp/temp_in${Date.now()}.mp4`;
  const outputPath = `mayel/temp/temp_out${Date.now()}.mp4`;
...
async function loadSession() {
    try {
        ...
        if (header !== "PRINCE-MD" || !b64data) {
            throw new Error("❌ Invalid session format. Expected 'Prince~.....'");
        }
