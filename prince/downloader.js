const { gmd, PrinceApkDl } = require("../mayel"),
       GIFTED_DLS = require('gifted-dls'),
       princeDls = new GIFTED_DLS();
...
gmd({
    ...
    try {
        const searchResponse = await gmdJson(
            `${PrinceTechApi}/search/yts?apikey=${PrinceApiKey}&query=${encodeURIComponent(q)}`
        );
        ...
        const handleResponse = async (event) => {
            ...
            try {
                ...
                const downloadResult = await princeDls.ytmp4(q, quality);
