...
• Community: ${gInfo.isCommunity ? '✅' : '❌'}
    ...
    await react("❌");
    await Prince.sendMessage(from, { text: "Failed to fetch group metadata." }, { quoted: mek });
  }
});
...
gmd({
  ...
  try {
    await Prince.groupParticipantsUpdate(from, [finalResult], 'demote'); 
...
gmd({
  ...
  try {
    await Prince.groupParticipantsUpdate(from, [finalResult], 'promote'); 
